      ...
      <div v-if="comments?.length">
        <div v-for="(comment, index) in comments" :key="index" class="w-full">
          <div class="w-full mt-2 pb-1 bg-slate-300 rounded-md text-sm">
            ...
            >
              <div
                class="w-full flex flex-wrap justify-start bg-slate-500 hover:bg-slate-600 p-1.5 gap-2"
              >
                <img
...
export default {
  ...
  setup(props) {
    ...
    function getComments(page = 1) {
      fetch(`https://forum-backend-cttc.onrender.com/user/comments/all/${props.id}/${page}`, {
        method: "GET",
        headers: {
