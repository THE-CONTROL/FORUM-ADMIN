<template>
  <div class="w-full text-slate-100">
    <div
      class="bg-slate-400 m-auto h-full rounded-md w-8/12 md:w-7/12 pb-4 rounded-tr-md"
    >
      <button
        class="bg-red-700 hover:bg-red-500 float-right font-semibold text-slate-100 p-1 rounded-tr-md"
        @click="showModalFalse1()"
      >
        <router-link to="/posts">X</router-link>
      </button>
      <h1 class="text-center text-2xl font-semibold pt-4 text-slate-900">
        Delete Post
      </h1>
      <p class="text-center mt-4 text-slate-900 px-2">
      ...
      <div class="w-full mt-4 flex flex-wrap justify-center">
        <button
          class="bg-green-700 hover:bg-green-500 p-1 ml-4 rounded-md text-sm"
          @click="deletePost()"
        >
          <router-link to="/posts">Yes</router-link>
        </button>
        <button
          class="bg-red-700 hover:bg-red-500 p-1 ml-4 rounded-md text-sm"
          @click="showModalFalse1()"
        >
...
export default {
  ...
  setup(props) {
    ...
    function getPostDetails() {
      fetch(`https://forum-backend-cttc.onrender.com/post/${props.postId}`, {
        method: "GET",
        headers: {
    ...
    function deletePost() {
      fetch(`https://forum-backend-cttc.onrender.com/admin/post/delete/${props.postId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("admin-access")}`,
    ...
    return { deletePost, route, router, getPostDetails, post };
  },
};
</script>
